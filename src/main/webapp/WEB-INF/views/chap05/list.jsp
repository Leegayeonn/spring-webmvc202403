...
<div id="wrap">
    ...
    <div class="top-section">
        ...
        <div class="amount">
            <div><a href="/board/list?pageNo=1&amout=6&type=${s.type}&keyword=${s.keyword}">6</a></div>
            <div><a href="/board/list?pageNo=1&amout=18&type=${s.type}&keyword=${s.keyword}">18</a></div>
            <div><a href="/board/list?pageNo=1&amout=30&type=${s.type}&keyword=${s.keyword}">30</a></div>
        </div>
            ...
            <ul class="pagination pagination-lg pagination-custom">
                ...
                <c:if test="${maker.page.pageNo != 1}">
                    <li class="page-item"><a class="page-link"
                        href="/board/list?pageNo=1&amout=${s.amount}&type=${s.type}&keyword=${s.keyword}">&lt;&lt;</a>
                    </li>
                </c:if>
                <c:if test="${maker.prev}">
                    <li class="page-item"><a class="page-link"
                        href="/board/list?pageNo=${maker.begin - 1}&amout=${s.amount}&type=${s.type}&keyword=${s.keyword}">prev</a>
                    </li>
                </c:if>
                ...
                <c:forEach var="i" begin="${maker.begin}" end="${maker.end}" step="1"> <!-- step="1"이면 생략가능함-->
                    <li data-page-num="${i}" class="page-item">
                        <a class="page-link"
                        href="/board/list?pageNo=${i}&amout=${s.amount}&type=${s.type}&keyword=${s.keyword}">${i}</a>
                    </li>
                </c:forEach>

                <c:if test="${maker.next}">
                    <li class="page-item"><a class="page-link"     
                        href="/board/list?pageNo=${maker.end + 1}&amout=${s.amount}&type=${s.type}&keyword=${s.keyword}">next</a>
                    </li>
                </c:if>

                <c:if test="${maker.page.pageNo != maker.finalPage}">
                    <li class="page-item"><a class="page-link"
                        href="/board/list?pageNo=${maker.finalPage}&amout=${s.amount}&type=${s.type}&keyword=${s.keyword}">&gt;&gt;</a>
                    </li>
                </c:if>
            </ul>
        </nav>
...
<script>
    ...
    $cardContainer.addEventListener('click', e => {
        ...
        if (e.target.matches('.card-btn-group *')) {
            ...
            // section 태그에 붙은 글번호를 읽어오자 (b.boardNo)
            // 이벤트가 발생한 타겟에서 가장 가까운 section.card를 지목해서 data-bno얻어오기 date- -> 얻어오려면 dataset사용
  ...
  function fixSearchOption() {
    ...
    $options.forEach($opt => {
        if ($opt.value === '${s.type}') {
            // option 태그에 selected 를 주면 그 option이 고정됨
