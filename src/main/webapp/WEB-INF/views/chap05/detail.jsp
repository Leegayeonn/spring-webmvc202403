...
<div id="wrap" class="form-container">
    ...
    <div class="buttons">
        <button class="list-btn" type="button"
                onclick="location.href='/board/list?pageNo=${s.pageNo}&amout=${s.amount}&type=${s.type}&keyword=${s.keyword}'">
            목록
        </button>
    </div>
        ...
        <div class="offset-md-1 col-md-10">
            ...
            <div class="card">
                ...
                <c:if test="${login == null}">
                    <a href="/members/sign-in">댓글작성은 로그인이 필요합니다</a>
                </c:if>    
    ...
    <script>
        ...
        function renderPage({begin, next, end, prev, page, finalPage}) { 
        ...
        function renderReplies(replyList) {
            ...
            if (replies !== null && replies.length > 0) {
                ...
                tag += "<div id='replyContent' class='card-body'>****** 무플방지 ***** 꺄륵 </div>";
            }
            // 댓글 수 렌더링
        ...
        function fetchGetReplies(pageNo = 1) { // 댓글 목록 요청시 페이지번호 전달(전달되지않으면 기본값 1)
            // fetch 함수를 통해 비동기통신 진행할때 GET 요청은 요청에 관련한 객체를 따로 전달하지않습니다.
            // method를 get 이라고 얘기하지않고, 데이터 전달 시에는 URL에 포함시켜서 전달.
            // 자바스크립트 문자열 안에 $중괄호를 쓰면 el로 인식, 템플릿 리터럴 문자를 쓰고싶으면 앞에 \를 붙여주세요
            fetch(`\${URL}\/\${bno}/page/\${pageNo}`)  // fetch(URL + '/' + bno)도 가능
                .then(res => res.json())
                .then(replyList => {
                    console.log(replyList);
                    // 서버로부터 전달받은 댓글 목록들을 화면에 그려야한다.
                    // 기존에는 model에 담아서 jsp로 전달했고, jsp쪽에서 el을 이용해서 화면에 뿌렸습니다
                ...
                fetch(URL, requestInfo)
                    ...
                    .then(res => {
                        ...
                        if (res.status === 200) {
                            ...
                            alert('입력값에 문제가 있습니다. 입력값 재확인이 필요합니다!');
                            return res.text();
                        }
                    })
                    .then(data => {
                        ...
                        // 댓글 목록 비동기 요청이 들어가야한다.
                        // 따로 함수로 빼주겠습니다 (등록 이후 뿐만 아니라 게시글 상세보기에 처음 들어왔을때에도 호출되어야함)
                        fetchGetReplies();
                    });
        ...
        function makeReplyRemoveClickHandler() {
            ...
            $replyData.onclick = e => {
                ...
                // 저 두개 이외에 이벤트가 발생하면 무시 -> return 굳이 쓸 필요없음.
            }
        }
