...
//        dto.setPassword(encodedPw);  -> 간략하게 표현가능->dto 엔터티 매개변수로 encoder 설정
    ...
    public LoginResult authenticate(LoginRequestDTO dto,
        ...
        if (dto.isAutoLogin()) {
            ...
            // 4. DB 에도 쿠키에 관련된 값들(랜덤한 세션아이디, 자동로그인 만료시간)을 갱신.
            ...
            ); // session_id, limit_time, account 까지 받아야함 -> AutoLoginDTO 생성
    ...
    public void maintainLoginState(HttpSession session, String account) {
        ...
        // 세션은 쿠키와 달리 모든데이터를 저장할 수 있으며 크기도 제한이 없습니다.
    ...
    public void autoLoginClear(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (c != null) {
            ...
            // 3. DB 에서도 세션아이디와 만료시간을 제거하자.
            memberMapper.saveAutoLogin(
                    AutoLoginDTO.builder()
                            .sessionId("none") // 세션아이디 지우기
                            .limitTime(LocalDateTime.now()) // 로그아웃한 현재 날짜 지정
    ...
    public void kakaoLogout(LoginUserResponseDTO dto, HttpSession session) {
        ...
        // 만약 access_token 의 값을 DB에 저장한 경우에는, 응답받은 id를 통해서
        // DB의 access_token 의 값을 null 로 만들어 주시면 됩니다.
