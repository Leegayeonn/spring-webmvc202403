...
//@Component 랑 같지만 Controller 역할을 할예정
...
public class ControllerV1 {
    ...
    // 스프링에서는 메서드별로 url 을 매핑하여 처리합니다.

//  원형:  @RequestMapping(value = "/food", method = RequestMethod.GET)
    @GetMapping("/food")
    public String food() {
        // 리턴문에는 어떤 jsp 로 포워딩할지 경로를 적습니다.
        // 경로를 작성할 때는 view resolver 세팅에 작성된 접두어, 접미어를 고려하여 씁니다.
        // /WEB-INF/views/~~~~~.jsp 사이에 경로값을 적으면됨
        return "chap01/food";
    }


    // ========================== 요청 파라미터 읽기 (클라이언트가 보낸 데이터) ================================
    // 1. HttpServletRequest 객체 이용하기 -> 전통적방식

//  원형: @RequestMapping(value = "/food", method = RequestMethod.POST)
//    @PostMapping("/food")
//    public String foodReg(HttpServletRequest request) { // 디스패쳐서블릿이 호출해주는것
